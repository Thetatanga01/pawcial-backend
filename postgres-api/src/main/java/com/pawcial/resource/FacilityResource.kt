...
class FacilityResource {
    ...
    @DELETE
    @Path("/{id}")
    @Operation(summary = "Tesisi sil (soft delete)")
    ...
    @Path("/{id}/hard-delete")
    @Operation(
        summary = "Tesisi kalıcı olarak sil (hard delete)",
        description = "Tesisi veritabanından kalıcı olarak siler. Sadece oluşturulduktan sonraki belirli süre içinde izin verilir."
    )
