...
class AssetResource {
    ...
    @DELETE
    @Path("/{id}")
    @Operation(summary = "Varlığı sil (soft delete)")
    ...
    @Path("/{id}/hard-delete")
    @Operation(
        summary = "Varlığı kalıcı olarak sil (hard delete)",
        description = "Varlığı veritabanından kalıcı olarak siler. Sadece oluşturulduktan sonraki belirli süre içinde izin verilir."
    )
