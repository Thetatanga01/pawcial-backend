...
class AnimalEventResource {
    ...
    @Operation(
        ...
        description = "Toggle the active status of an animal event (soft delete). Note: Events older than 30 minutes cannot be deleted (read-only)."
    )
    @APIResponses(
        APIResponse(responseCode = "204", description = "Event deleted successfully"),
        APIResponse(responseCode = "400", description = "Event is read-only (older than 30 minutes)"),
        APIResponse(responseCode = "404", description = "Event not found")
    )
    fun deleteEvent(
        @Parameter(description = "Event ID", required = true)
